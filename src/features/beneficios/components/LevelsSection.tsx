...
/**
 * Lista de níveis de fidelidade, exibidos como cards com medalha/ícone.
 * Usa dados do hook de benefícios.
 */
export const LevelsSection = React.memo(() => {
  const { niveis } = useBeneficios();
  const medalColors = ["#b08d57", "#c0c0c0", "#ffd700", "#4fd1c5"];
  return (
    <div className="my-4">
      <h2 className="text-[#181711] text-[22px] font-bold leading-tight tracking-[-0.015em] px-1 pb-3 pt-5">Níveis de Fidelidade</h2>
      <div className="flex flex-col gap-3">
        {niveis.map((item, i) => (
          <Card key={item.name} className="flex items-center gap-4 bg-white px-4 min-h-[72px] py-2">
